...
class hdict(dict[str, VT]):
    ...
    >>> d["f"] = f
    >>> cache = {}
    >>> d = d >> apply(field("f"), field("x"), y=default(3), nonexistent_parameter=7)("w3", "v3").cached(cache)  # TODO: nonexistent parameter should raise an exception
    ...
    >>> d = hdict(w=6) >> (apply(f, _.w, y=3)(z="z").cached(cache) >> apply(g, x=_[1,2,3,...,5], y=7)("w", "v")).sample()
